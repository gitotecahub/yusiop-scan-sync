...
BEGIN
  ...
  IF NEW.qr_card_id IS NOT NULL THEN
    ...
    IF FOUND AND v_card.download_credits > 0 THEN
      IF v_card.origin = 'physical' THEN
        v_physical_xaf := CASE v_card.card_type WHEN 'premium' THEN 7000 ELSE 3500 END;
        v_gross := ROUND(v_physical_xaf::numeric / v_card.download_credits);
...
-- Recalculate existing earnings from real downloads
...
DECLARE r record;
BEGIN
  FOR r IN SELECT id FROM public.user_downloads WHERE download_type = 'real' ORDER BY downloaded_at ASC LOOP
